      ...
      <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        {(['bars', 'wave', 'ring'] as Mode[]).map(m => (
          <button
            key={m}
            onClick={() => setMode(m)}
            className={`px-2 py-1 text-[9px] uppercase font-medium rounded transition-colors ${
              mode === m 
...
const WaveVisualizer = memo(function WaveVisualizer({ analyser }: { analyser: Tone.Analyser }) {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const normalized = Math.max(0, (val + 100) / 100);
      const x = (i / count) * 6 - 3;
      const y = Math.sin(i * 0.2 + time) * 0.3 + normalized * 1.5 - 0.5;
      positions[i * 3] = x;
      positions[i * 3 + 1] = y;
      positions[i * 3 + 2] = 0;
    }
  ...
  return (
    <points ref={ref}>
      <bufferGeometry />
      <pointsMaterial size={0.08} color="#1db954" transparent opacity={0.8} />
    </points>
  );
});
