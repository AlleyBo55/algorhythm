...
export const DeckComponent = memo(function DeckComponent({ id, expanded, small }: DeckProps) {
  ...
  const handlePlay = useCallback(() => { deck.play(); setIsPlaying(true); }, [deck]);
  const handlePause = useCallback(() => { deck.pause(); setIsPlaying(false); }, [deck]);
  const handleStop = useCallback(() => { deck.stop(); setIsPlaying(false); }, [deck]);
    ...
    <div className={cn(
      ...
      <div className="flex-1 min-h-0 p-4 flex flex-col">
        {!trackName ? (
          ...
          >
            ...
            <span className="text-sm text-zinc-500 group-hover:text-zinc-400 transition-colors">Load Track</span>
          </button>
        ) : (
            ...
            <div className="flex-1 flex items-center justify-center">
              ...
              >
                ...
                <div className={cn(
                  ...
                  <div className={cn("w-2 h-2 rounded-full", deckColor === 'cyan' ? "bg-cyan-400" : "bg-amber-400")} />
                </div>
              </div>
            </div>
      ...
      {trackName && (
        <div className="h-16 px-4 flex items-center justify-center gap-2 border-t border-zinc-800/50">
          ...
          <TransportButton onClick={isPlaying ? handlePause : handlePlay} primary active={isPlaying} title={isPlaying ? "Pause" : "Play"}>
            {isPlaying ? (
...
const TransportButton = memo(function TransportButton({
  onClick, children, primary, active, title,
}: {
  onClick: () => void; children: React.ReactNode; primary?: boolean; active?: boolean; title?: string;
}) {
  return (
    <button onClick={onClick} title={title} className={cn(
      "flex items-center justify-center rounded-full transition-all",
      primary ? "w-12 h-12 bg-white text-black hover:scale-105 active:scale-95" : "w-10 h-10 bg-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-700",
      active && primary && "bg-[#1db954]"
    )}>{children}</button>
  );
});

const EQKnob = memo(function EQKnob({ label, value, onChange }: { label: string; value: number; onChange: (v: number) => void }) {
  return (
    <div className="flex flex-col items-center gap-1">
      <span className="text-[10px] font-medium text-zinc-500">{label}</span>
      <input type="range" min="-12" max="12" step="1" value={value} onChange={e => onChange(Number(e.target.value))}
      ...
      <span className={cn("text-[10px] font-mono tabular-nums", value === 0 ? "text-zinc-600" : value > 0 ? "text-cyan-400" : "text-amber-400")}>
        {value > 0 ? '+' : ''}{value}
      </span>
    </div>
  );
