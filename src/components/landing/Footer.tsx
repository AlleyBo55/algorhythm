...
export const Footer = memo(function Footer({ theme }: FooterProps) {
  return (
    <footer className="relative py-12 sm:py-16 px-4 sm:px-6 border-t border-zinc-800/50">
...
const FooterContent = memo(function FooterContent({ theme }: { theme: SectionTheme }) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 mb-10 sm:mb-12">
      <BrandColumn theme={theme} />
      {Object.entries(FOOTER_LINKS).map(([category, links]) => (
        <LinkColumn key={category} category={category} links={links} theme={theme} />
      ))}
    </div>
  );
});

const BrandColumn = memo(function BrandColumn({ theme }: { theme: SectionTheme }) {
  return (
...
const LinkColumn = memo(function LinkColumn({ 
  category, 
  links,
  theme,
}: { 
      ...
      <ul className="space-y-2">
        {links.map((link) => (
          <li key={link.label}>
            <Link 
              href={link.href} 
              className="text-sm text-zinc-500 transition-colors duration-300"
...
const FooterBottom = memo(function FooterBottom({ theme }: { theme: SectionTheme }) {
  return (
    <div className="pt-8 border-t border-zinc-800/50 flex flex-col sm:flex-row justify-between items-center gap-4">
      <p className="text-sm text-zinc-600 text-center sm:text-left">
        Made with 💚 for musicians who code
      </p>
      <div className="flex items-center gap-4">
        <SocialLink href="#" label="GitHub" theme={theme}>
        ...
        <SocialLink href="#" label="Discord" theme={theme}>
          <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
        ...
        <SocialLink href="#" label="Twitter" theme={theme}>
          <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
