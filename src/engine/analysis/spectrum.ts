...
export class SpectrumAnalyzer {
  ...
  getData(): SpectrumData {
    // Use type assertion to handle ArrayBufferLike vs ArrayBuffer mismatch
    this.analyser.getFloatFrequencyData(this.frequencyData as unknown as Float32Array<ArrayBuffer>);
    this.analyser.getFloatTimeDomainData(this.waveformData as unknown as Float32Array<ArrayBuffer>);
    ...
    for (let i = 0; i < this.frequencyData.length; i++) {
      frequencies[i] = this.frequencyData[i];
    }
    
    const waveform = new Float32Array(this.waveformData.length);
    for (let i = 0; i < this.waveformData.length; i++) {
      waveform[i] = this.waveformData[i];
    }
